...
export const getSpecificAdmin= async(req,res,next)=>{
  const {adminId}=req.params;
  const admin = await userModel.findOne({_id:adminId,role:'Admin'});
  if(!admin){
    return next(new Error(`Admin is not found!!`,{cause:404}));
  }
...
export const createAdmin= async(req,res,next)=>{
   ...
   //     return res.status(409).json({message:"email already exists"});
    ...
    const RLink =`${req.protocol}://${req.headers.host}/admin/newConfirmEmail/${refreshToken}`;
    ...
    <head>
      ...
      }import { hash } from './../../../Services/hashAndCompare';
                    ...
                    <tr>
                      <td align="center" bgcolor="#ffffff" style="padding: 12px;">
                        <table border="0" cellpadding="0" cellspacing="0">
                          ...
                          <a href="${RLink}" target="_blank" style="display: inline-block; padding: 16px 36px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; color: #ffffff; text-decoration: none; border-radius: 6px;">Send New Email</a> 
    ...
    const createAdmin = await userModel.create({userName,email,password:hashPassword,role});
    return res.status(201).json({message:"Done",user:createAdmin._id});
  }
  export const confirmEmail = asyncHandler(async(req,res)=>{
    ...
    {
        // return res.status(200).redirect(`${process.env.F_URL}`);
        return res.status(200).json(`Your email is verified successfully`);
    }else{
        return next(new Error(`The account is not registered 
        or it has already verified successfully`),{cause:409}); } })
...
export const newConfirmEmail =asyncHandler(async(req,res)=>{
  ...
  export const updateAdmin=async(req,res,next)=>{
    ...
    return next(new Error(`You Try To Enter While You're Not An Admin!!`),{cause:403});
  }
}
  ...
  export const deleteAdmin=async(req,res,next)=>{
   ...
   if(!admin){
    ...
    const deletedAdmin= await userModel.findOneAndDelete({_id:adminId,role:'Admin'},{new: true});
    return res.status(200).json({message:"Success",deletedAdmin});
   }
