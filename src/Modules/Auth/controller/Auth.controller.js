...
export const signup= async (req,res,next)=>{
   ...
   //     return res.status(409).json({message:"email already exists"});
    ...
    const createUser = await userModel.create({userName,email,password:hashPassword,role});
...
export const login = async(req,res,next)=>{
        ...
        if(!user){
            ...
            if(!user.confirmEmail){
                return next(new Error("plz verify your email",{cause:409}));
            }
            ...
            if(!match){
                ...
                const refreshToken =generateToken({id:user._id, role:user.role},process.env.LOGIN_TOKEN,'1h');
...
if(!user)
{
  return next(new Error('Invalid Account!!',{cause:409}));
}
let code = customAlphabet('123456789abABCD',4);
code= code();
// return res.json(code);
const userUpdated= await userModel.findOneAndUpdate({email},{forgetCode:code},{new:true});
const html= `<p>Code is ${code}</p>`;
await sendEmail(email,`Forgot Password`,html);
return res.status(200).json({message:"Success",userUpdated});})
export const forgetPassword=asyncHandler(async(req,res,next)=>{
const {email,password,code}=req.body;
const user = await userModel.findOne({email});
if(!user){
  return next(new Error('Invalid User Account!!',{cause:409}));
}
